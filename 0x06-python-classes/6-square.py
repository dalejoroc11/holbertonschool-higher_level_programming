...
a TypeError exception with the messagesize must be an integer
if size is less than 0, raise a ValueError
exception with the message size mustbe >= 0
...
class Square:
    ...
    def __init__(self, size=0, position=(0, 0)):
        """__init1__ the data with error handling"""
        self.size = size
    ...
    @property
    def position(self):
        """Returns current square's position"""
        return self.__position

    @property
    def size(self):
        """Returns size Square instance"""
        return self.__size
